...
Add the following section after the DENSE ARTIFACT/SPELL COLUMNS instruction: 'CARD NAME vs UI LABEL DISAMBIGUATION: Some cards share names with UI elements or column labels. For example, a card displayed as a column header or section label (e.g., a card name appearing at the top of a column in italic or bold) must still be counted as a card if it has a visible card frame, title bar, or stack beneath it. Do NOT skip a card simply because its name also appears as a UI label. Additionally, for columns where the column header total (e.g., 10, 5, 4) is shown, verify that every distinct card title bar visible within that column — including any card whose name matches the column label — is individually listed. If a card appears as both a UI label AND has a physical card stack beneath it, count the stack using Method B. SPECIAL CASE — SINGLE-COPY CARDS IN MULTI-CARD COLUMNS: In artifact/spell columns, many cards appear only once (1 copy, no stack). Do not skip them. Each distinct title bar = a separate card entry with quantity 1 unless stacked. Re-read each column from top to bottom at least twice to catch single-copy cards that may be visually thin or partially obscured.'`;
